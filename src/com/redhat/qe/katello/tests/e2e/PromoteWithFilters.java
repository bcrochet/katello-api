...
public class PromoteWithFilters extends KatelloCliTestScript{
	...
	public void test_promoteToDevNoSync(){
		...
		KatelloChangeset cs = new KatelloChangeset(clienttasks, this.cs1, this.org, this.env);
		cs.create();
	...
	public void test_createApplyFilter(){
		...
		KatelloFilter filter = new KatelloFilter(clienttasks, this.filter, this.org, this.env, PACKAGES_TO_BLACKLIST);
	...
	public void test_promoteToDeSyncedAndFiltered(){
		...
		KatelloChangeset cs = new KatelloChangeset(clienttasks, this.cs2, this.org, this.env);
		cs.create();
	...
	public void test_packagesPromotedDev(){
		String cmd_packListDev = KATELLO_CLI_DEFAULT +" package list --org \"%s\" --environment \"%s\" --product \"%s\" --repo \"%s\" | grep -E \"%s\" | wc -l";
